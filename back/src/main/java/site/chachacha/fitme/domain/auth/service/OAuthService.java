...
import site.chachacha.fitme.domain.auth.dto.KakaoTokenResponse;
import site.chachacha.fitme.domain.auth.dto.KakaoUserResponse;
...
public class OAuthService {
    ...
    public KakaoOAuthResponse requestKakao(String provider, String code, String requestUrl)
        throws IOException {
    ...
    private KakaoTokenResponse requestKakaoTokenResponse(String provider, String code,
        ...
        try (OutputStream os = connection.getOutputStream()) {
            String encoded = String.format(
                "grant_type=authorization_code&client_id=%s&redirect_uri=%s/auth/login/oauth2/code/%s&code=%s&client_secret=%s",
                kakaoClientId,
                requestUrl,
                provider,
                code,
        ...
        if (responseCode == 400) {
            ...
            while ((errorInputLine = errorReader.readLine()) != null) {
                errorBuffer.append(errorInputLine);
            }
            errorReader.close();
        ...
        // respons -> JSON
        KakaoTokenResponse kakaoTokenResponse = objectMapper.readValue(response,
    ...
    public void requestLogOut(Token tokenEntity) throws IOException {
        ...
        connection.setRequestProperty("Authorization",
            "Bearer " + tokenEntity.getResourceAccessToken());
        connection.setDoOutput(true);
    ...
    private KakaoUserResponse requestKakaoUserInfo(KakaoTokenResponse kakaoTokenResponse)
        throws IOException {
        // 요청
