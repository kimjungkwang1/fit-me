...
import site.chachacha.fitme.auth.dto.KakaoOAuthResponse;
import site.chachacha.fitme.auth.dto.KakaoTokenResponse;
import site.chachacha.fitme.auth.dto.KakaoUserResponse;
...
public class OAuthService {
    ...
    private KakaoTokenResponse requestKakaoTokenResponse(String provider, String code, String requestUrl)
        ...
        try (OutputStream os = connection.getOutputStream()) {
            String encoded = String.format("grant_type=authorization_code&client_id=%s&redirect_uri=%s/api/auth/login/oauth2/code/%s&code=%s&client_secret=%s",
                kakaoClientId,
                requestUrl,
                provider,
                code,
